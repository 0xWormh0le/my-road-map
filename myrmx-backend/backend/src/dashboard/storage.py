...
class _Local(local):
    ...
    def __init__(self, storage):
        ...
        self.session = boto3.session.Session(profile_name=storage.settings.AWS_PROFILE_NAME)
...
class PublicS3Storage(S3Storage):
    """
    An S3 storage which always stores files as publicly accessible
    """

    def _setup(self):
        super()._setup()
        setattr(self.settings, "AWS_S3_BUCKET_AUTH", False)
